...
local player1ScoreNum
local player2ScoreNum
local player1ScoreZero
local player2ScoreZero
local num1
local num2
local winnerText
local counter = 0
...
local   pongChannel				 = audio.loadSound( "pong.wav" )
local 	scoreChannel			 = audio.loadSound( "score.wav")
...
function ship.new(name)
  ...
  physics.addBody(self.body,"dynamic", {radius = 25, bounce = 0.1})
...
function ship:fire()
	...
	physics.addBody( self.laser, "dynamic", {radius = 20, filter = laserCollisionFilter} )
	...
	self.laser:applyLinearImpulse(50*forceMag*self.xComp, 50*forceMag*self.yComp, self.laser.x, self.laser.y)
end
...
local function createAsteroid()
	...
	physics.addBody(newAsteroid,"dynamic", { radius = 95, density = 0, bounce = 1.0, filter = asteroidCollisionFilter})
	...
	newAsteroid.x = display.contentWidth/2
		...
		newAsteroid:setLinearVelocity(math.random(-90,90),math.random(-90,90))
	newAsteroid:applyTorque(-4,4)
	counter = 3
end

local function makeAsteroidBumpers()
	local fudgeNum = 20 + 2 -- extra positioning when rock is moved
	
	asteroidBumper["top"] = display.newRect( display.contentWidth/2,0 - fudgeNum, display.contentWidth + fudgeNum *2, 10 )	
	...
	physics.addBody( asteroidBumper["top"], "dynamic", {density = 20, friction = 20,bumperCollisionFilter})

	asteroidBumper["left"] = display.newRect( 0 - fudgeNum, display.contentHeight/2, 10, display.contentHeight + fudgeNum*2 )
	...
	physics.addBody( asteroidBumper["left"], "dynamic", {density = 20, friction = 20,bumperCollisionFilter})

	asteroidBumper["bottom"] = display.newRect( display.contentWidth/2, display.contentHeight + fudgeNum, display.contentWidth + fudgeNum * 2, 10 )
	...
	physics.addBody( asteroidBumper["bottom"], "dynamic", {density = 20, friction = 20,bumperCollisionFilter})

	asteroidBumper["right"] = display.newRect( display.contentWidth + fudgeNum, display.contentHeight/2, 10, display.contentHeight+fudgeNum*2 )
	...
	physics.addBody( asteroidBumper["right"], "dynamic", {density = 20, friction = 20,bumperCollisionFilter})
end
...
local function onCollision( event )
	...
	if ( event.phase == "began" ) then
		...
		if (obj1.type == "asteroidBumper" and (obj1.name == "left" or obj1.name == "right") and obj2.type == "asteroid" and scored == false) then
        	...
        	scored = true
        	explosion(obj2.x,obj2.y,1.0)
        	display.remove(obj2)
        	obj2 = nil
			local pongChannel1 = audio.play( pongChannel, { channel=12 })
				  --audio.setVolume( 1.0, { channel=12 } )
        	local function score()
        		...
        		if obj1.name == "left" then 
	        		...
	        		if player2Score == 1 then
	        			display.remove(player2ScoreZero)
	        			num2 = "one.png"
	        			player2ScoreNum = display.newImageRect( uiGroup, num2, 400, 400)
						player2ScoreNum.x = display.contentWidth - 300
						player2ScoreNum.y = 110
	        		elseif player2Score == 2 then
	        			display.remove(player2ScoreNum)
	        			num2 = "two.png"
	        			player2ScoreNum = display.newImageRect( uiGroup, num2, 400, 400)
						player2ScoreNum.x = display.contentWidth - 300
						player2ScoreNum.y = 110
	        	...
	        	elseif obj1.name == "right" then
	        		...
	        		if player1Score == 1 then
	        			display.remove(player1ScoreZero)
	        			num1 = "one.png"
	        			player1ScoreNum = display.newImageRect( uiGroup, num1, 400, 400)
						player1ScoreNum.x = 300
						player1ScoreNum.y = 110
	        		elseif player1Score == 2 then
	        			display.remove(player1ScoreNum)
	        			num1 = "two.png"
	        			player1ScoreNum = display.newImageRect( uiGroup, num1, 400, 400)
						player1ScoreNum.x = 300
						player1ScoreNum.y = 110
        		...
        		if(player1Score == 3 or player2Score == 3) then
					...
					local displayText = display.newText(uiGroup, "New Pong being dropped in", display.contentWidth/2, display.contentHeight/2 - 15, inputText, 30)
						local function tmr(i)
							--print(counter)
							if ( counter == 3) then
								--print(" first")
								timerText = display.newText(uiGroup, counter, display.contentWidth/2, display.contentHeight/2 + 15, inputText, 30)
							elseif ( counter > 0) then 
								...
								--print("remove")
								display.remove(timerText)
								display.remove(displayText)
								timer.performWithDelay(1, restoreAsteroid)
							end
							counter = counter - 1
						end
						timer.performWithDelay(1000,tmr,4)
		        	end
	        	end
	        	timer.performWithDelay (1, score )

        elseif (obj1.type == "asteroidBumper" and (obj1.name == "top" or obj1.name == "bottom") and obj2.type == "asteroid") then
		local scoreChannel1 = audio.play( scoreChannel, { channel=13 })
			 -- audio.setVolume( 1.0, { channel=13 } )
...
function scene:create( event )
	...
	local goals = display.newImageRect( backGroup, "pongGoals.png", 1920, 1080 )
	goals.x = display.contentCenterX
	goals.y = display.contentCenterY

	local player1ScoreText = display.newImageRect( uiGroup, "player1Score.png", 400, 400 )
	player1ScoreText.x = 300
	player1ScoreText.y = 110

	local player2ScoreText = display.newImageRect( uiGroup, "player2Score.png", 400, 400 )
	player2ScoreText.x = display.contentWidth - 300
	player2ScoreText.y = 110

	player1ScoreZero = display.newImageRect( uiGroup, "zero.png", 400, 400 )
	player1ScoreZero.x = 300
	player1ScoreZero.y = 110

	player2ScoreZero = display.newImageRect( uiGroup, "zero.png", 400, 400 ) 
	player2ScoreZero.x = display.contentWidth - 300
	player2ScoreZero.y = 110
