...
local objectSheet18 = graphics.newImageSheet("blackHole1.png", sheetOptions18)

local sheetOptions19 =
{
	frames = 
	{		
		{
...
local objectSheet19 = graphics.newImageSheet("blackHole2.png", sheetOptions19)

local sheetOptions20 =
{
	frames = 
	{		
		{
...
local objectSheet20 = graphics.newImageSheet("blackHole3.png", sheetOptions20)

local sheetOptions21 =
{
	frames = 
	{		
		{
...
local objectSheet21 = graphics.newImageSheet("blackHole4.png", sheetOptions21)
...
local blackhole = display.newCircle(display.contentCenterX, display.contentCenterY, 500)
blackhole.alpha = 0
physics.addBody( blackhole, "static", { isSensor=true, radius = 500} )
blackhole.name = "blackhole"
blackhole.type = "blackhole"
--physics.setContinuous( true )

local singularity = display.newCircle(display.contentCenterX, display.contentCenterY, 60)
singularity.alpha = 0
physics.addBody( singularity, "static", { isSensor=true, radius = 60} )
...
local function onFrameEvent()
	...
	for i = #blackholeCollision,1,-1 do
    	obj = blackholeCollision[i]
    	distance = 500/(2*math.sqrt(math.pow((obj.y - display.contentCenterY),2) + math.pow((obj.x - display.contentCenterX),2)))
    	objAngle = math.atan2((display.contentCenterY-obj.y),(display.contentCenterX-obj.x))
		objXComp = math.cos(objAngle)
		objYComp = math.sin(objAngle)
		obj:applyLinearImpulse(distance*forceMag*objXComp, distance*forceMag*objYComp, obj.x, obj.y)
	end
...
local function restoreShip1()
	if(player1.lives > 0) then
		...
		player1.body.x = math.random(100,300)
...
local function restoreShip2()
	if(player2.lives > 0) then			
		...
		player2.body.x = math.random(1620,1820)
...
local function onCollision( event )
	...
	if(obj1.type == "blackhole") then
		if ( event.phase ~= "ended" ) then
			table.insert(blackholeCollision, obj2)
		else
			for i = #blackholeCollision, 1, -1 do
				if ( blackholeCollision[i] == obj2)  then
					blackholeCollision[i].destroy()
					table.remove(blackholeCollision, i)
				end
			end
		end
	end

	if(obj1.type == "singularity") then
		if ( event.phase == "began" ) then
			...
			if(obj2.type ~= "ship") then
				...
				display.remove(obj2)
				explosion(display.contentCenterX, display.contentCenterY,0.3)
			else
				if(obj2.name == "ship1" and player1.died == false) then
				...
				elseif(obj2.name == "ship2" and player2.died == false) then
					explosion(obj2.x,obj2.y, 0.7)
					local explosionChannel2 = audio.play( explosionChannel, { channel=6 } )
					player2.health = 0
					player2.died = true
	...
	if ( event.phase == "began" ) then
		...
		elseif ((obj1.type == "laser" and obj2.name == "smallAsteroid" and obj2.hit == false) or
			...
			for i = #blackholeCollision, 1, -1 do
				if ( blackholeCollision[i] == obj1)  then
					table.remove(blackholeCollision, i)
				end
			end
			for i = #blackholeCollision, 1, -1 do
...
function scene:create( event )
	...
	blackhole1 = display.newImage(mainGroup, objectSheet18, display.contentCenterX, display.contentCenterY)
	blackhole1.alpha = 0.6
	blackhole1.x = display.contentCenterX
	blackhole1.y = display.contentCenterY
	blackhole2 = display.newImage(mainGroup, objectSheet19, display.contentCenterX, display.contentCenterY)
	blackhole2.alpha = 0.6
	blackhole2.x = display.contentCenterX
	blackhole2.y = display.contentCenterY
	blackhole3 = display.newImage(mainGroup, objectSheet20, display.contentCenterX, display.contentCenterY)
	blackhole3.alpha = 0.6
	blackhole3.x = display.contentCenterX
	blackhole3.y = display.contentCenterY
	blackhole4 = display.newImage(mainGroup, objectSheet21, display.contentCenterX, display.contentCenterY)
	blackhole4.alpha = 0.6
	blackhole4.x = display.contentCenterX
	blackhole4.y = display.contentCenterY
